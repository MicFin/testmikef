...
var onCreateTeams = function onCreateTeams(event) {
  event.preventDefault();
  var data = getFormFields(event.target);
  console.log(data);
  api.createTeams(data).then(ui.createTeamsSuccess).catch(ui.createTeamsFailure);
};
var onUpdateTeams = function onUpdateTeams(event) {
  event.preventDefault();
  var data = getFormFields(event.target);
  api.updateTeams(data).then(ui.updateTeamsSuccess).catch(ui.updateTeamsFailure);
};
var onGetTeams = function onGetTeams(event) {
  event.preventDefault();
  var data = getFormFields(event.target);
  api.getTeams(data).then(ui.getTeamsSuccess).catch(ui.getTeamsFailure);
};
var onDestroyTeam = function onDestroyTeam(event) {
  event.preventDefault();
  var data = getFormFields(event.target);
  api.destroyTeam(data).then(ui.destroyTeamSuccess).catch(ui.destroyTeamFailure);
};
...
var createTeams = function createTeams(data) {
  return $.ajax({
...
var updateTeams = function updateTeams(data) {
  // console.log(data)
...
var getTeams = function getTeams(data) {
  return $.ajax({
    url: config.apiUrl + '/teams',
    method: 'GET',
    headers: {
...
var destroyTeam = function destroyTeam(data) {
...
// Teams ui below
var createTeamsSuccess = function createTeamsSuccess(data) {
  $('#message').text('Successfully created team');
  $('#message').css('background-color', 'green');
  $('#create-teams').trigger('reset');
};
var createTeamsFailure = function createTeamsFailure() {
  $('#message').text('Failure to create team');
  $('#message').css('background-color', 'red');
  $('#create-teams').trigger('reset');
};

var updateTeamsSuccess = function updateTeamsSuccess(data) {
  $('#message').text('Successfully updated team');
  $('#message').css('background-color', 'green');
  $('#update-teams').trigger('reset');
};
var updateTeamsFailure = function updateTeamsFailure() {
  $('#message').text('Failure to update team');
  $('#message').css('background-color', 'red');
  $('#update-teams').trigger('reset');
};
var getTeamsSuccess = function getTeamsSuccess(data) {
  console.log(data);
  $('#team-list').text('');
  $('#message2').text('');
  $('#message2').css('background', 'none');
  for (var i = 0; i < data.teams.length; i++) {
    var teamName = data.teams[i].team_name;
...
var getTeamsFailure = function getTeamsFailure() {
  $('#message2').text('Failure to get teams');
  $('#message2').css('background-color', 'red');
};
var destroyTeamSuccess = function destroyTeamSuccess(data) {
  $('#message').text('Successfully deleted team');
  $('#message').css('background-color', 'green');
  $('#delete-teams').trigger('reset');
};
var destroyTeamFailure = function destroyTeamFailure() {
...
module.exports = {
  ...
  createTeamsSuccess: createTeamsSuccess,
  createTeamsFailure: createTeamsFailure,
  updateTeamsSuccess: updateTeamsSuccess,
  updateTeamsFailure: updateTeamsFailure,
  getTeamsSuccess: getTeamsSuccess,
  getTeamsFailure: getTeamsFailure,
  destroyTeamSuccess: destroyTeamSuccess,
  destroyTeamFailure: destroyTeamFailure
};
/* WEBPACK VAR INJECTION */}.call(exports, __webpack_require__(0)))
